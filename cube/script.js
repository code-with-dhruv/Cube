...
function animateRotation(face, cw, currentTime) {
	var k = .3 * (face % 2 * 2 - 1) * (2 * cw - 1),
			qubes = Array(16).fill(pieces[face]).map(function (value, index) {
...
function shuffleCube(numShuffles) {
	for (let i = 0; i < numShuffles; i++) {
	  const face = Math.floor(Math.random() * 6);
	  const rotationCount = Math.floor(Math.random() * 10) + 1;
	  animateRotation(face, true, Date.now());
	}
  };
